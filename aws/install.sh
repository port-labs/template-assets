...
then
    ...
    echo ""
    aws iam create-policy --policy-name "${EXPORTER_IAM_POLICY_NAME}" --policy-document "file://${temp_dir}/policy.json" \ 
    || aws iam create-policy-version --policy-arn arn:aws:iam::${AWS_ACCOUNT_ID}:policy/${EXPORTER_IAM_POLICY_NAME} --policy-document "file://${temp_dir}/policy.json" --set-as-default true || exit
else
    echo "Policy exists"
fi
