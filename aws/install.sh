...
#   EXPORTER_BUCKET_NAME - The bucket name to create for the exporter configuration (default="port-aws-exporter-{128bit hash value}")
...
EXPORTER_BUCKET_NAME=${EXPORTER_BUCKET_NAME:-"port-aws-exporter-$(echo -n "${PORT_CLIENT_SECRET}" | md5)"}
...
EXPORTER_IAM_POLICY_ARN="arn:aws:iam::$(aws sts get-caller-identity | jq -r ".Account"):policy\/${EXPORTER_IAM_POLICY_NAME}"
...
aws s3api wait object-exists --bucket "${EXPORTER_BUCKET_NAME}" --key "config.json"

# Checks if AWS SAM CLI is installed
if ! command -v sam &> /dev/null
then
    echo "sam command is not exists, can't tail live logs of exporter's lambda"
    echo ""
