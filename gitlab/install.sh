...
#   SUPPRESS_WEBHOOK_CREATION - If set to true, the script will not create a webhook in GitLab (optional)
###################################################
...
SUPPRESS_WEBHOOK_CREATION = "${SUPPRESS_WEBHOOK_CREATION:-"false"}"
...
then
  ...
  python3 "${temp_dir}/gitlab_exporter.py" "${PORT_CLIENT_ID}" "${PORT_CLIENT_SECRET}" "${GITLAB_API_TOKEN}" "${GROUPS_TO_REPOS}" "${GITLAB_API_URL}" "${SUPPRESS_WEBHOOK_CREATION}"
elif command -v python &>/dev/null 
then
  echo "Python is installed, Running script..."
  python -m pip install requests
  python "${temp_dir}/gitlab_exporter.py" "${PORT_CLIENT_ID}" "${PORT_CLIENT_SECRET}" "${GITLAB_API_TOKEN}" "${GROUPS_TO_REPOS}" "${GITLAB_API_URL}" "${SUPPRESS_WEBHOOK_CREATION}"
else
